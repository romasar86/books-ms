const assert = require("chai").assert,
    ...
    Books = require("./../../../app/models/books");


describe("Models/BooksModel", () => {
    describe("#findById()", () => {
        it("it should reject with error because of invalid id", (done) => {
            const id = "eiuwieu32487",
                ...
                books = new Books(dbMock);
            books.findById(id).then(done).catch( err => {
        ...
        it("it should resolve with proper data" , (done) => {
            const id = "testId",
                ...
                books = new Books(dbMock);
            books.findById(id).then(actualDoc => {
                ...
                assert.isTrue(dbMock.getCollection.calledWith(books.collectionName));
    ...
    describe("#delete()", () => {
        it("it should reject with error because of invalid id", (done) => {
            const id = "eiuwieu32487",
                ...
                books = new Books(dbMock);
            books.delete(id).then(done).catch( err => {
                assert.instanceOf(err, Error);
                done();
            });
        });
        ...
        it("it should resolve with proper data" , (done) => {
            const id = "testId",
                ...
                books = new Books(dbMock);
            books.delete(id).then(actualDoc => {
                assert.deepEqual(actualDoc, doc);
                assert.isTrue(dbMock.ObjectID.isValid.calledOnce);
                assert.isTrue(dbMock.ObjectID.isValid.calledWith(id));
                assert.isTrue(dbMock.getCollection.calledOnce);
    ...
    describe("#findAll()", () => {
        it("it should reject with error", (done) => {
            const dbMock = {
                ...
                books = new Books(dbMock);
            books.findAll().then(done).catch( err => {
                assert.instanceOf(err, Error);
                done();
            });
        });
    ...
    describe("#errorHandler()", () => {
        it("it should return error instance", () => {
            const books = new Books(),
                type = "testType",
                message = "testMessage",
                err = books.errorHandler(type, message);
    ...
    describe("#insert()", () => {
        it("it should reject with validation error", (done) => {
            const testErrorMessage = "testError",
                ...
                testDoc = { test: "doc" },
                books = new Books({}, validatorMock);
            books.insert(testDoc).then(done).catch(err => {
                assert.instanceOf(err, Error);
        ...
        it("it should resolve properly", (done) => {
            const validatorMock = {
                ...
                testDoc = { test: "doc" },
                books = new Books(dbMock, validatorMock);
            books.insert(testDoc).then(() => {
                assert.isTrue(dbMock.getCollection.calledOnce);
                assert.isTrue(dbMock.getCollection.calledWith(books.collectionName));
    ...
    describe("#update()", () => {
        it("it should reject with validation error -- id is not ObjectID", (done) => {
            const dbMock = {
                ...
                testDoc = { test: "doc" },
                books = new Books(dbMock);
            books.update(id, testDoc).then(done).catch(err => {
                assert.instanceOf(err, Error);
                assert.isTrue(dbMock.ObjectID.isValid.calledOnce);
                assert.isTrue(dbMock.ObjectID.isValid.calledWith(id));
                assert.equal(err.type, "ValidationError");
